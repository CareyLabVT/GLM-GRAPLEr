...
curr_directory = "C:/Users/arian_000/Documents/Summer 2017/Carey Lab/108k Sim Results/"
...
varo6 = "NIT_amm"
varo7 = "PHS_frp"
varos = c(varo1, varo2, varo3, varo4, varo5, varo6, varo7) # create a vector of the vars 
...
for (i in 1:length(sim_summary[,1])) {
  ...
  if (goodguy) {
    if (firstOne) {
      ...
      conditions = c(sim_summary[i,5], sim_summary[i,9], sim_summary[i,13], sim_summary[i,17], sim_summary[i,21], sim_summary[i,25], sim_summary[i,29])
      firstOne = FALSE
    }
    else { 
      foldernames = cbind(foldernames, c(as.character(sim_summary$Sim1_1[i])))
      conditions = rbind(conditions, cbind(sim_summary[i,5], sim_summary[i,9], sim_summary[i,13], sim_summary[i,17], sim_summary[i,21], sim_summary[i,25], sim_summary[i,29]))
    }
  }
}

colnames(conditions) = c("AirTemp", "WindSpeed", "RelHum", "Rain", "NIT_amm", "flow", "PHS_frp")

firstOne = TRUE
for (j in 1:length(foldernames)) {
  copiesconds = rbind(conditions[j,], conditions[j,], conditions[j,], conditions[j,], conditions[j,], conditions[j,])
  if (firstOne) {
    yearly <- cbind(conditions[j,], read.csv(paste0(curr_directory, "/Sims/", foldernames[j], "/Results/Sim__.csv")))
    annual <- cbind(copiesconds, read.csv(paste0(curr_directory, "/Sims/", foldernames[j], "/Results/Data_By_Time.csv")))
    firstOne = FALSE
  }
  else {
    yearly <- rbind(yearly, cbind(conditions[j,], read.csv(paste0(curr_directory, "Sims/", foldernames[j], "/Results/Sim__.csv"))))
    annual <- rbind(annual, cbind(copiesconds, read.csv(paste0(curr_directory, "Sims/", foldernames[j], "/Results/Data_By_Time.csv"))))
  }
}
...
for (i in 1:length(annual[,1])) { 
  evaluate = TRUE
  if (annual$datesNew[i] != 2010) { #&& annual[i, columnnum] == eval(as.name(paste0((as.name(var1)), "basel"))) && annual[i, columnnum2] == eval(as.name(paste0((as.name(var2)), "basel")))) { # 0 is for temp or rain 1.04 for flow
...
for (i in 1:length(annual[,1])) {
  ...
  if (annual$datesNew[i] != 2010) { 
    ...
    if (((as.numeric(as.character(annual$max_0_2_diatoms[i])) - meanbaselinediatom) > threshold) || ((as.numeric(as.character(annual$max_0_2_diatoms[i])) - meanbaselinediatom) < (-threshold)) ) {
      ...
      listcrossedthreshold = rbind(listcrossedthreshold, annual[i,1:8])
    }
    if (((as.numeric(as.character(annual$date_max_0_2_diatoms[i])) - meanbaselinediatomday) > thresholddays) || ((as.numeric(as.character(annual$date_max_0_2_diatoms[i])) - meanbaselinediatomday) < (-thresholddays)) ) {
      ...
      listcrossedthreshold = rbind(listcrossedthreshold, annual[i,1:8]) #eventually get rid of this
...
write.csv(dataframe_overall, paste0(varnames, "_elsewhere0_Counts_Threshold_Diatoms.csv"), row.names = FALSE)
write.csv(listcrossedthreshold, paste0(varnames, "_list_dates_Threshold_Diatoms.csv"), row.names = FALSE)
