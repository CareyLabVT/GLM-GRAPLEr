...
# Written by Arianna Krinos, last edits by KJF on 23 May 2017, AIK 28 May 2017
...
depth <- 30
fractional <- 'false'
#print(" Choose a nutrient (PHS_frp, NIT_amm, OXY_oxy, or NIT_nit): ", quote = FALSE)
#nutrientVar = readline(prompt = "Enter it here: ")
#print("Choose a depth (0,5,10,15,20,25,30,33 for Sunapee): ", quote = FALSE) 
#depth = readline("Enter it here: ") 
#print("Toggle the division by the baseline to be 'true' or 'false' (no quotes)")
#fractional = readline(prompt = "Enter true or false here: ") 
if (nutrientVar == "PHS_frp") {
  gapVect = c(0,2,4,14,16,18)
  gapVar = c(5,17,20)
...
#plot(example3[,8] / example3[,8] ~ example3[,1], type = 'l', xlab = "Time", ylab = "Fraction of baseline", main = nutrientVar, ylim = c(0,4), xaxt='n')#ylim = c(0, 0.15), type = 'l')
nutr_values = (example3[,colnum] / (baseline_nutrient[,colnum]))

# Plot proportion from baseline or concentration over time ####
if (fractional) {
  nutr_values = (example3[,colnum] / (baseline_nutrient[,colnum]))
} else {
  nutr_values = example3[,colnum]
}
...
if (fractional) {
  gap.plot(nutr_values, gap = gapVar, gap.axis = 'y', type = 'l', xlab = "Time", ylab = "Fraction of baseline", main = paste(nutrientVar, 'at', depth,'m'), ytics = gapVect,xaxt='n',xtics = c(0))#ylim = c(0, 0.15), type = 'l')
} else {
  plot(nutr_values, type = 'l', xlab = "Time", ylab = "Concentration (mg/L)", main = paste(nutrientVar, 'at', depth,'m'), xaxt='n')
...
for (j in 1:length(sim_folders)) { # Iterate through sim folders
  for (k in 1:length(folders_to_find2[,1])) {
    if (sim_folders[j] == folders_to_find2[k,1]) {
      ...
      if (fractional) {
        gap.plot(nutr_values, gap = gapVar, gap.axis = 'y', type = 'l', xlab = "Time", col = cols[counter3], ylab = "Fraction of baseline", main = paste(nutrientVar, 'at', depth,'m'), ytics = gapVect,xaxt='n',xtics = c(0), add = TRUE,lty= ltys[counter3])#ylim = c(0, 0.15), type = 'l')
      } else {
        lines(nutr_values, col = cols[counter3], lty= ltys[counter3])
      }
      gap.plot(nutr_values, gap = gapVar, gap.axis = 'y', type = 'l', xlab = "Time", col = cols[counter3], ylab = "Fraction of baseline", main = paste(nutrientVar, 'at', depth,'m'), ytics = gapVect,xaxt='n',xtics = c(0), add = TRUE,lty= ltys[counter3])#ylim = c(0, 0.15), type = 'l')
      ...
      legendlabels[counter3] = allconditions[counter3 + 1, 2] # Needs to be changed for whatever the toggle var is
      counter3 = counter3 + 1
      break
    }
  }
...
legend("top",c("setup",legendlabels), fill = c("black", cols), ncol=9, cex=0.75)
